...
var import_enhanced_debugging = require("./enhanced-debugging");
...
const install = ({ richMockErrors } = {}) => {
  options.richMockErrors = richMockErrors != null ? richMockErrors : import__.configs.test.richMockErrors;
...
const lookupResponse = (request) => {
  if (options.richMockErrors) return (0, import_enhanced_debugging.lookupResponse)(store, options)(request);
  const mocks = store.map((mock) => mock.toMockRequest());
