...
import { sortedUrl, toSortedQueryString, filterKeys } from "./mock-utils.mjs";
var MATCHED_AS_UNDEFINED_IN_MOCK = "MATCHED_AS_UNDEFINED_IN_MOCK";
var MATCHED_BY_FUNCTION = "MATCHED_BY_FUNCTION";
var MISMATCH_BY_FUNCTION = "MISMATCHED_BY_FUNCTION";
...
MockRequest.prototype = {
  ...
  bodyMatchRequest(request) {
    ...
    if (this.bodyFunction) {
      const match2 = this.body(request.body());
      const value = match2 ? MATCHED_BY_FUNCTION : MISMATCH_BY_FUNCTION;
      return { match: match2, mockValue: value, requestValue: value };
    }
  ...
  urlMatchRequest(request) {
    if (this.urlFunction) {
      const match2 = Boolean(this.url(request.url(), request.params()));
      const value = match2 ? MATCHED_BY_FUNCTION : MISMATCH_BY_FUNCTION;
      return { match: match2, mockValue: value, requestValue: value };
    }
  ...
  headersMatchRequest(request) {
    ...
    if (this.headersFunction) {
      const match2 = this.headers(request.headers());
      const value = match2 ? MATCHED_BY_FUNCTION : MISMATCH_BY_FUNCTION;
      return { match: match2, mockValue: value, requestValue: value };
    }
