...
import { bt as I18nT, h as computedAsync, yt as useI18n } from "./_MapCache-CrDDrJ83.js";
import { Bi as N8nButton_default, Hi as N8nIcon_default, Ri as N8nCallout_default, Ti as N8nTooltip_default, pi as N8nLink_default, xi as useRouter, zi as N8nText_default } from "./src-BsVsetYf.js";
import { t as __plugin_vue_export_helper_default } from "./_plugin-vue_export-helper-DltO58Gh.js";
import { Bt as useNDVStore, C as useNodeTypesStore, Ei as executionDataToJson, It as useNodeHelpers, Lt as useTelemetry, Or as require_FileSaver_min, Pt as environments_store_default, Tr as useMessage, Vt as injectWorkflowState, at as generateCodeForPrompt, ct as useDataSchema, lt as useFlattenSchema, pa as nodeViewEventBus, r as useUIStore, ra as isEmpty, s as useWorkflowsStore, xr as useExternalHooks, xs as useSettingsStore, yr as useToast } from "./users.store-yKax6vGx.js";
import { An as MANUAL_TRIGGER_NODE_TYPE, Fi as createResultError, Gc as EnterpriseEditionFeature, N as PLACEHOLDER_FILLED_AT_EXECUTION_TIME, S as SCHEMA_PREVIEW_DOCS_URL, Sr as WEBHOOK_NODE_TYPE, Wt as CHAT_TRIGGER_NODE_TYPE, ao as NodeConnectionTypes, bs as MODAL_CONFIRM, do as AI_TRANSFORM_NODE_TYPE, hn as FORM_TRIGGER_NODE_TYPE, io as DateTime, is as BINARY_DATA_VIEW_MODAL_KEY, lo as AI_TRANSFORM_CODE_GENERATED_FOR_PROMPT, mi as require_pick, ps as FROM_AI_PARAMETERS_MODAL_KEY, uo as AI_TRANSFORM_JS_CODE, zo as ApplicationError } from "./constants-CtA9jyV-.js";
import { r as useRootStore } from "./_baseOrderBy-B5lXBmMF.js";
import { i as Ln, o as gu, r as estree_exports, t as NDVEmptyState_default } from "./NDVEmptyState-BuTIQOIa.js";
...
import { t as useCalloutHelpers } from "./useCalloutHelpers-35vAQH3i.js";
import { n as MappingPill_default, t as useTelemetryContext } from "./useTelemetryContext-DroScxId.js";
import { i as useSchemaPreviewStore, t as useRunWorkflow } from "./useRunWorkflow-BIyMxaO6.js";
	...
	setup(__props, { emit: __emit }) {
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("div", _hoisted_1$3, [createBaseVNode("div", _hoisted_2$2, [
				...
				__props.nodeType ? (openBlock(), createBlock(NodeIcon_default, {
					key: 0,
				...
				createBaseVNode("div", _hoisted_3$1, [createTextVNode(toDisplayString(__props.title) + " ", 1), __props.info ? (openBlock(), createElementBlock("span", _hoisted_4, toDisplayString(__props.info), 1)) : createCommentVNode("", true)]),
				...
				__props.itemCount && !__props.lastSuccessfulPreview ? (openBlock(), createElementBlock("div", _hoisted_5, toDisplayString(unref(i18n).baseText("ndv.output.items", { interpolate: { count: __props.itemCount } })), 1)) : __props.preview ? (openBlock(), createElementBlock("div", _hoisted_6, toDisplayString(unref(i18n).baseText("dataMapping.schemaView.previewNode")), 1)) : createCommentVNode("", true)
			]), __props.preview && !__props.collapsed ? (openBlock(), createElementBlock("div", {
				key: 0,
				class: "notice",
				"data-test-id": "schema-preview-warning",
				onClick: _cache[3] || (_cache[3] = withModifiers(() => {}, ["stop"]))
			}, [__props.lastSuccessfulPreview ? (openBlock(), createBlock(unref(I18nT), {
				key: 0,
				keypath: "dataMapping.schemaView.previewLastExecution",
				scope: "global"
			}, {
				execute: withCtx(() => [createVNode(unref(N8nLink_default), {
					onClick: _cache[1] || (_cache[1] = () => emit("execute", __props.title)),
					size: "small",
					bold: ""
				}, {
				...
				execute: withCtx(() => [createVNode(unref(N8nLink_default), {
					onClick: _cache[2] || (_cache[2] = () => emit("execute", __props.title)),
					size: "small",
					bold: ""
				}, {
...
var VirtualSchemaItem_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	emits: ["click"],
	setup(__props, { emit: __emit }) {
		useCssVars((_ctx) => ({ "bae8c388": _ctx.level }));
		const props$1 = __props;
		const i18n = useI18n();
		async function downloadBinaryData() {
			if (!props$1.binaryData) return;
			try {
				const { id, fileName, mimeType, fileExtension } = props$1.binaryData;
				const url = useWorkflowsStore().getBinaryUrl(id, "download", fileName ?? "", mimeType);
				let name = fileName ?? "file";
				if (!name.includes(".") && fileExtension) name = [name, fileExtension].join(".");
				const response = await fetch(url);
				if (!response.ok) throw new Error("Error downloading file");
				(0, import_FileSaver_min.saveAs)(await response.blob(), name);
			} catch (error) {
		...
		function viewBinaryData() {
			useUIStore().openModalWithData({
				...
				data: { binaryData: props$1.binaryData }
			});
		}
		const emit = __emit;
		return (_ctx, _cache) => {
			...
			return openBlock(), createElementBlock("div", {
				class: normalizeClass(["schema-item", { draggable: __props.draggable }]),
				"data-test-id": "run-data-schema-item"
			}, [
				createBaseVNode("div", _hoisted_1$2, [__props.collapsable ? (openBlock(), createElementBlock("div", {
					key: 0,
					class: "toggle",
				...
				__props.title ? (openBlock(), createElementBlock("div", {
					key: 0,
					"data-name": __props.title,
					"data-path": __props.path,
					"data-depth": __props.depth,
					"data-nest-level": __props.level,
					"data-value": __props.expression,
					"data-node-type": __props.nodeType,
					"data-target": !__props.locked && "mappable",
					"data-node-name": __props.nodeName,
					"data-run-index": __props.runIndex,
					class: normalizeClass(["pill", {
						"pill--highlight": __props.highlight,
						"pill--preview": __props.preview,
						"pill--locked": __props.locked
					}]),
				...
				__props.locked ? (openBlock(), createBlock(unref(N8nTooltip_default), {
					key: 1,
					disabled: !__props.lockedTooltip,
					"content-class": _ctx.$style.tooltip
				}, {
					content: withCtx(() => [withDirectives(createBaseVNode("span", null, null, 512), [[_directive_n8n_html, __props.lockedTooltip]])]),
					...
					_: 1
				}, 8, ["disabled", "content-class"])) : createCommentVNode("", true),
				createVNode(TextWithHighlights_default, {
					"data-test-id": "run-data-schema-item-value",
					class: "text",
					content: __props.value,
					search: props$1.search
				}, null, 8, ["content", "search"]),
				props$1.binaryData && !__props.preview ? (openBlock(), createElementBlock("div", _hoisted_3, [createBaseVNode("div", {
					class: normalizeClass(["pill binary-control-pill", {
						"pill--highlight": __props.highlight,
						"pill--preview": __props.preview,
						"pill--locked": __props.locked
					}]),
...
var VirtualSchemaItem_default = /* @__PURE__ */ __plugin_vue_export_helper_default(VirtualSchemaItem_vue_vue_type_script_setup_true_lang_default, [["__cssModules", { "$style": VirtualSchemaItem_vue_vue_type_style_index_1_lang_module_default }], ["__scopeId", "data-v-5ae9241a"]]);
	...
	setup(__props, { emit: __emit }) {
		...
		return (_ctx, _cache) => {
			return openBlock(), createBlock(unref(N8nTooltip_default), {
				placement: __props.tooltipPlacement ?? "right",
				...
				default: withCtx(() => [createVNode(unref(N8nButton_default), mergeProps(_ctx.$attrs, {
					loading: isLoading.value && __props.showLoadingSpinner,
					disabled: __props.disabled || !!disabledHint.value || isLoading.value && !__props.showLoadingSpinner,
					label: buttonLabel.value,
					type: __props.type,
					size: __props.size,
					icon: buttonIcon.value,
					square: __props.square,
					"transparent-background": __props.transparent,
...
var VirtualSchema_default = /* @__PURE__ */ __plugin_vue_export_helper_default(/* @__PURE__ */ defineComponent({
	...
	setup(__props, { emit: __emit }) {
		...
		const onCalloutDismiss = async (calloutId) => {
		...
		const items = computed(() => {
			let allItems;
			if (isDebugging.value || props$1.paneType === "output") {
				allItems = flattenNodeSchema.value;
				if (props$1.node?.type === "n8n-nodes-base.merge" && props$1.paneType === "output" && props$1.data && props$1.data.length > 1) allItems = [{
					id: `${props$1.node.name}-mergeNotice`,
					type: "callout",
					level: 0,
					message: i18n.baseText("dataMapping.schemaView.mergeNotice"),
					theme: "info"
				}, ...allItems];
			} else allItems = flattenedNodes.value.concat(contextItems.value);
			return allItems.filter((item) => {
				if (item.type === "callout") return !isCalloutDismissed(item.id);
				return true;
			});
		});
		...
		const unwatchItems = watch(items, (newItems) => {
			const headers = newItems.length < 2 ? [] : newItems.filter((item) => item.type === "header").slice(1).map((item) => item.id);
			const binaries = newItems.filter((item) => item.type === "item" && item.binaryData).map((item) => item.id);
				...
				default: withCtx(() => [createVNode(unref(script$1), {
					...
					default: withCtx(({ item, index, active }) => [createVNode(unref(script), {
						...
						default: withCtx(() => [item.type === "header" ? (openBlock(), createBlock(VirtualSchemaHeader_default, mergeProps({ key: 0 }, item, {
							...
							draggable: __props.mappingEnabled,
							collapsed: unref(closedNodes).has(item.id),
						...
						}, null, 4)), [[_directive_n8n_html, item.message]]) : item.type === "callout" ? (openBlock(), createElementBlock("div", {
							key: 4,
							...
							trailingContent: withCtx(() => [createVNode(unref(N8nIcon_default), {
								...
								onClick: ($event) => onCalloutDismiss(item.id)
							}, null, 8, ["title", "onClick"])]),
							default: withCtx(() => [createTextVNode(toDisplayString(item.message) + " ", 1)]),
							_: 2
						}, 1032, ["theme"])], 4)) : item.type === "empty" ? (openBlock(), createElementBlock("div", {
							key: 5,
							class: "empty-schema",
							style: normalizeStyle({ "--schema-level": item.level })
