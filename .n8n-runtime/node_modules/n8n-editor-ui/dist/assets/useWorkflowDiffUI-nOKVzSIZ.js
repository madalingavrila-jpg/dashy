import { C as computed, Cn as toDisplayString, D as createElementBlock, E as createCommentVNode, Gt as unref, I as guardReactiveProps, It as ref, M as createVNode, P as defineComponent, T as createBlock, Ut as toValue, W as mergeProps, _ as Fragment, _t as watch, at as renderSlot, bn as normalizeStyle, bt as withCtx, c as useCssModule, et as openBlock, it as renderList, j as createTextVNode, l as useCssVars, vn as normalizeClass, vt as watchEffect, w as createBaseVNode, yn as normalizeProps, zt as shallowRef } from "./vue.runtime.esm-bundler-XtMkEjzB.js";
import { H as createEventHook, U as createInjectionState, yt as useI18n } from "./_MapCache-CrDDrJ83.js";
import { Ai as N8nIconButton_default, Li as N8nHeading_default, N as N8nResizeWrapper_default, mt as createEventBus, zi as N8nText_default } from "./src-BsVsetYf.js";
import { t as __plugin_vue_export_helper_default } from "./_plugin-vue_export-helper-DltO58Gh.js";
import { C as useNodeTypesStore, Jn as _sfc_main$d, ir as useVueFlow, r as useUIStore, s as useWorkflowsStore } from "./users.store-yKax6vGx.js";
import { fi as NodeDiffStatus, mr as STICKY_NODE_TYPE, pi as compareWorkflowsNodes } from "./constants-CtA9jyV-.js";
import { t as NodeIcon_default } from "./NodeIcon-Be4Kw4K9.js";
import { c as CanvasBackground_default, n as Canvas_default, o as CanvasEdge_default, r as CanvasNode_default, t as useCanvasMapping } from "./useCanvasMapping-CQVKZrsr.js";
var DiffBadge_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	__name: "DiffBadge",
	props: { type: {} },
	setup(__props) {
		useCssVars((_ctx) => ({ "v2f23a409": backgroundColor.value }));
		const props = __props;
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("div", { class: normalizeClass(_ctx.$style.diffBadge) }, toDisplayString(label.value), 3);
		};
	}
});
...
ae.prototype = {
	diff: function(r, l) {
		...
		var u = (f = h.timeout) !== null && f !== void 0 ? f : Infinity, d = Date.now() + u, g = [{
			oldPos: -1,
			lastComponent: void 0
		}], p = this.extractCommon(g[0], l, r, 0);
		if (g[0].oldPos + 1 >= o && p + 1 >= s) return i([{
			value: this.join(l),
			count: l.length
		}]);
		...
		function E() {
			for (var w = Math.max(_, -a); w <= Math.min(m, a); w += 2) {
				...
				if (!O || R && T.oldPos + 1 < M.oldPos ? D = t.addToPath(M, !0, void 0, 0) : D = t.addToPath(T, void 0, !0, 1), p = t.extractCommon(D, l, r, w), D.oldPos + 1 >= o && p + 1 >= s) return i(Yn(t, D.lastComponent, l, r, t.useLongestToken));
	...
	extractCommon: function(r, l, f, h) {
		for (var n = l.length, t = f.length, i = r.oldPos, s = i - h, o = 0; s + 1 < n && i + 1 < t && this.equals(l[s + 1], f[i + 1]);) s++, i++, o++;
...
function ze(e, r, l, f, h) {
	...
	if (nt.call(e) === "[object Array]") {
		for (r.push(e), t = new Array(e.length), l.push(t), n = 0; n < e.length; n += 1) t[n] = ze(e[n], r, l, f, h);
		return r.pop(), l.pop(), t;
	}
	if (e && e.toJSON && (e = e.toJSON()), Ie(e) === "object" && e !== null) {
		r.push(e), t = {}, l.push(t);
		var i = [], s;
		for (s in e) e.hasOwnProperty(s) && i.push(s);
		for (i.sort(), n = 0; n < i.length; n += 1) s = i[n], t[s] = ze(e[s], r, l, f, s);
		r.pop(), l.pop();
	} else t = e;
	return t;
}
...
(function(e) {
	...
	r.Diff = function(n, t) {
		...
		if (n == t) return n ? [new r.Diff(h, n)] : [];
		...
		if (d) {
			...
			return v.concat([new r.Diff(h, E)], w);
		}
			...
			switch (a[c][0]) {
				...
				case h:
					if (u >= 1 && d >= 1) {
						...
						for (var _ = this.diff_main(g, p, !1, i), m = _.length - 1; m >= 0; m--) a.splice(c, 0, _[m]);
						c = c + _.length;
					}
					d = 0, u = 0, g = "", p = "";
					break;
		...
		for (var _ = s - o, m = _ % 2 != 0, E = 0, v = 0, w = 0, D = 0, T = 0; T < a && !((/* @__PURE__ */ new Date()).getTime() > i); T++) {
			for (var M = -T + E; M <= T - v; M += 2) {
				...
				if (d[R] = L, L > s) v += 2;
					...
					if (P >= 0 && P < u && g[P] != -1) {
						var $ = s - g[P];
						if (L >= $) return this.diff_bisectSplit_(n, t, L, O, i);
					}
				}
			}
			for (var K = -T + w; K <= T - D; K += 2) {
				...
				if (g[P] = $, $ > s) D += 2;
					...
					if (R >= 0 && R < u && d[R] != -1) {
						var L = d[R], O = c + L - R;
						if ($ = s - $, L >= $) return this.diff_bisectSplit_(n, t, L, O, i);
					}
				}
			}
		}
...
`, p), _ == -1 && (_ = d.length - 1);
		...
		for (var i = 0; i < n.length; i++) {
			for (var s = n[i][1], o = [], a = 0; a < s.length; a++) o[a] = t[s.charCodeAt(a)];
			n[i][1] = o.join("");
		}
	}, r.prototype.diff_commonPrefix = function(n, t) {
		if (!n || !t || n.charAt(0) != t.charAt(0)) return 0;
		for (var i = 0, s = Math.min(n.length, t.length), o = s, a = 0; i < o;) n.substring(a, o) == t.substring(a, o) ? (i = o, a = i) : s = o, o = Math.floor((s - i) / 2 + i);
		return o;
	}, r.prototype.diff_commonSuffix = function(n, t) {
		if (!n || !t || n.charAt(n.length - 1) != t.charAt(t.length - 1)) return 0;
		for (var i = 0, s = Math.min(n.length, t.length), o = s, a = 0; i < o;) n.substring(n.length - o, n.length - a) == t.substring(t.length - o, t.length - a) ? (i = o, a = i) : s = o, o = Math.floor((s - i) / 2 + i);
		return o;
		...
		if (i.length < 4 || s.length * 2 < i.length) return null;
		var o = this;
		function a(v, w, D) {
		...
		var c = a(i, s, Math.ceil(i.length / 4)), u = a(i, s, Math.ceil(i.length / 2)), d;
		if (!c && !u) return null;
		...
		for (var t = !1, i = [], s = 0, o = null, a = 0, c = 0, u = 0, d = 0, g = 0; a < n.length;) n[a][0] == h ? (i[s++] = a, c = d, u = g, d = 0, g = 0, o = n[a][1]) : (n[a][0] == f ? d += n[a][1].length : g += n[a][1].length, o && o.length <= Math.max(c, u) && o.length <= Math.max(d, g) && (n.splice(i[s - 1], 0, new r.Diff(l, o)), n[i[s - 1] + 1][0] = f, s--, s--, a = s > 0 ? i[s - 1] : -1, c = 0, u = 0, d = 0, g = 0, o = null, t = !0)), a++;
		for (t && this.diff_cleanupMerge(n), this.diff_cleanupSemanticLossless(n), a = 1; a < n.length;) {
			if (n[a - 1][0] == l && n[a][0] == f) {
				...
				m >= E ? (m >= p.length / 2 || m >= _.length / 2) && (n.splice(a, 0, new r.Diff(h, _.substring(0, m))), n[a - 1][1] = p.substring(0, p.length - m), n[a + 1][1] = _.substring(m), a++) : (E >= p.length / 2 || E >= _.length / 2) && (n.splice(a, 0, new r.Diff(h, p.substring(0, E))), n[a - 1][0] = f, n[a - 1][1] = _.substring(0, _.length - E), n[a + 1][0] = l, n[a + 1][1] = p.substring(E), a++), a++;
			}
			a++;
		}
		...
		for (var t = !1, i = [], s = 0, o = null, a = 0, c = !1, u = !1, d = !1, g = !1; a < n.length;) n[a][0] == h ? (n[a][1].length < this.Diff_EditCost && (d || g) ? (i[s++] = a, c = d, u = g, o = n[a][1]) : (s = 0, o = null), d = g = !1) : (n[a][0] == l ? g = !0 : d = !0, o && (c && u && d && g || o.length < this.Diff_EditCost / 2 && c + u + d + g == 3) && (n.splice(i[s - 1], 0, new r.Diff(l, o)), n[i[s - 1] + 1][0] = f, s--, o = null, c && u ? (d = g = !0, s = 0) : (s--, a = s > 0 ? i[s - 1] : -1, d = g = !1), t = !0)), a++;
		t && this.diff_cleanupMerge(n);
	}, r.prototype.diff_cleanupMerge = function(n) {
		n.push(new r.Diff(h, ""));
		for (var t = 0, i = 0, s = 0, o = "", a = "", c; t < n.length;) switch (n[t][0]) {
			case f:
				s++, a += n[t][1], t++;
				break;
			case l:
				...
				break;
			case h:
				i + s > 1 ? (i !== 0 && s !== 0 && (c = this.diff_commonPrefix(a, o), c !== 0 && (t - i - s > 0 && n[t - i - s - 1][0] == h ? n[t - i - s - 1][1] += a.substring(0, c) : (n.splice(0, 0, new r.Diff(h, a.substring(0, c))), t++), a = a.substring(c), o = o.substring(c)), c = this.diff_commonSuffix(a, o), c !== 0 && (n[t][1] = a.substring(a.length - c) + n[t][1], a = a.substring(0, a.length - c), o = o.substring(0, o.length - c))), t -= i + s, n.splice(t, i + s), o.length && (n.splice(t, 0, new r.Diff(l, o)), t++), a.length && (n.splice(t, 0, new r.Diff(f, a)), t++), t++) : t !== 0 && n[t - 1][0] == h ? (n[t - 1][1] += n[t][1], n.splice(t, 1)) : t++, s = 0, i = 0, o = "", a = "";
				break;
		}
		...
		for (c = 0; c < n.length && (n[c][0] !== f && (i += n[c][1].length), n[c][0] !== l && (s += n[c][1].length), !(i > t)); c++) o = i, a = s;
		return n.length != c && n[c][0] === l ? a : a + (t - o);
	}, r.prototype.diff_prettyHtml = function(n) {
		for (var t = [], i = /&/g, s = /</g, o = />/g, a = /\n/g, c = 0; c < n.length; c++) {
			var u = n[c][0], g = n[c][1].replace(i, "&amp;").replace(s, "&lt;").replace(o, "&gt;").replace(a, "&para;<br>");
			switch (u) {
				case f:
		...
		for (var t = [], i = 0; i < n.length; i++) n[i][0] !== f && (t[i] = n[i][1]);
		return t.join("");
	}, r.prototype.diff_text2 = function(n) {
		for (var t = [], i = 0; i < n.length; i++) n[i][0] !== l && (t[i] = n[i][1]);
		return t.join("");
		...
		for (var t = [], i = 0; i < n.length; i++) switch (n[i][0]) {
			case f:
				t[i] = "+" + encodeURI(n[i][1]);
				break;
			case l:
		...
		for (var g, p, _ = t.length + n.length, m, E = 0; E < t.length; E++) {
			for (g = 0, p = _; g < p;) a(E, i + p) <= c ? g = p : _ = p, p = Math.floor((_ - g) / 2 + g);
			_ = p;
			...
			for (var T = w; T >= v; T--) {
				...
				if (E === 0 ? D[T] = (D[T + 1] << 1 | 1) & M : D[T] = (D[T + 1] << 1 | 1) & M | ((m[T + 1] | m[T]) << 1 | 1) | m[T + 1], D[T] & d) {
					var R = a(E, T - 1);
					if (R <= c) if (c = R, u = T - 1, u > i) v = Math.max(1, 2 * i - u);
					else break;
				}
			}
			if (a(E + 1, i) > c) break;
		...
		for (var t = {}, i = 0; i < n.length; i++) t[n.charAt(i)] = 0;
		for (var i = 0; i < n.length; i++) t[n.charAt(i)] |= 1 << n.length - i - 1;
		return t;
	}, r.prototype.patch_addContext_ = function(n, t) {
		if (t.length != 0) {
			if (n.start2 === null) throw Error("patch not initialized");
			for (var i = t.substring(n.start2, n.start2 + n.length1), s = 0; t.indexOf(i) != t.lastIndexOf(i) && i.length < this.Match_MaxBits - this.Patch_Margin - this.Patch_Margin;) s += this.Patch_Margin, i = t.substring(n.start2 - s, n.start2 + n.length1 + s);
			s += this.Patch_Margin;
		...
		var s, o;
		if (typeof n == "string" && typeof t == "string" && typeof i == "undefined") s = n, o = this.diff_main(s, t, !0), o.length > 2 && (this.diff_cleanupSemantic(o), this.diff_cleanupEfficiency(o));
		else if (n && typeof n == "object" && typeof t == "undefined" && typeof i == "undefined") o = n, s = this.diff_text1(o);
		else if (typeof n == "string" && t && typeof t == "object" && typeof i == "undefined") s = n, o = t;
		else if (typeof n == "string" && typeof t == "string" && i && typeof i == "object") s = n, o = i;
				...
				if (g == -1 ? p = t.substring(d, d + u.length) : p = t.substring(d, g + this.Match_MaxBits), u == p) t = t.substring(0, d) + this.diff_text2(n[a].diffs) + t.substring(d + u.length);
					...
					if (u.length > this.Match_MaxBits && this.diff_levenshtein(_) / u.length > this.Patch_DeleteThreshold) o[a] = !1;
						...
						for (var m = 0, E, v = 0; v < n[a].diffs.length; v++) {
							var w = n[a].diffs[v];
							w[0] !== h && (E = this.diff_xIndex(_, m)), w[0] === f ? t = t.substring(0, d + E) + w[1] + t.substring(d + E) : w[0] === l && (t = t.substring(0, d + E) + t.substring(d + this.diff_xIndex(_, m + w[1].length))), w[0] !== l && (m += w[1].length);
						}
					}
				}
			}
		...
		var o = n[0], a = o.diffs;
		if (a.length == 0 || a[0][0] != h) a.unshift(new r.Diff(h, i)), o.start1 -= t, o.start2 -= t, o.length1 += t, o.length2 += t;
		...
		if (o = n[n.length - 1], a = o.diffs, a.length == 0 || a[a.length - 1][0] != h) a.push(new r.Diff(h, i)), o.length1 += t, o.length2 += t;
		...
		for (var t = this.Match_MaxBits, i = 0; i < n.length; i++) if (!(n[i].length1 <= t)) {
			var s = n[i];
			n.splice(i--, 1);
			for (var o = s.start1, a = s.start2, c = ""; s.diffs.length !== 0;) {
				...
				for (u.start1 = o - c.length, u.start2 = a - c.length, c !== "" && (u.length1 = u.length2 = c.length, u.diffs.push(new r.Diff(h, c))); s.diffs.length !== 0 && u.length1 < t - this.Patch_Margin;) {
					...
					g === f ? (u.length2 += p.length, a += p.length, u.diffs.push(s.diffs.shift()), d = !1) : g === l && u.diffs.length == 1 && u.diffs[0][0] == h && p.length > 2 * t ? (u.length1 += p.length, o += p.length, d = !1, u.diffs.push(new r.Diff(g, p)), s.diffs.shift()) : (p = p.substring(0, t - u.length1 - this.Patch_Margin), u.length1 += p.length, o += p.length, g === h ? (u.length2 += p.length, a += p.length) : d = !1, u.diffs.push(new r.Diff(g, p)), p == s.diffs[0][1] ? s.diffs.shift() : s.diffs[0][1] = s.diffs[0][1].substring(p.length));
				}
		...
		for (var t = [], i = 0; i < n.length; i++) t[i] = n[i];
		return t.join("");
...
`), s = 0, o = /^@@ -(\d+),?(\d*) \+(\d+),?(\d*) @@$/; s < i.length;) {
			...
			for (t.push(c), c.start1 = parseInt(a[1], 10), a[2] === "" ? (c.start1--, c.length1 = 1) : a[2] == "0" ? c.length1 = 0 : (c.start1--, c.length1 = parseInt(a[2], 10)), c.start2 = parseInt(a[3], 10), a[4] === "" ? (c.start2--, c.length2 = 1) : a[4] == "0" ? c.length2 = 0 : (c.start2--, c.length2 = parseInt(a[4], 10)), s++; s < i.length;) {
				...
				else if (u == " ") c.diffs.push(new r.Diff(h, d));
				else {
					if (u == "@") break;
...
var rt = "</span>", ln = (e) => !!e.scope, it = (e, { prefix: r }) => {
	...
	if (e.includes(".")) {
		const l = e.split(".");
		return [`${r}${l.shift()}`, ...l.map((f, h) => `${f}${"_".repeat(h + 1)}`)].join(" ");
	}
	return `${r}${e}`;
};
var st = class {
...
var gt = /\b\B/, An = "[a-zA-Z]\\w*", Xe = "[a-zA-Z_]\\w*", Mn = "\\b\\d+(\\.\\d+)?", Tn = "(-?)(\\b0[xX][a-fA-F0-9]+|(\\b\\d+(\\.\\d*)?|\\.\\d+)([eE][-+]?\\d+)?)", Dn = "\\b(0b[01]+)", ht = "!|!=|!==|%|%=|&|&&|&=|\\*|\\*=|\\+|\\+=|,|-|-=|/=|/|:|;|<<|<<=|<=|<|===|==|=|>>>=|>>=|>=|>>>|>>|>|\\?|\\[|\\{|\\(|\\^|\\^=|\\||\\|=|\\|\\||~", dt = (e = {}) => {
	...
	const h = Ze("I", "a", "is", "so", "us", "to", "at", "if", "in", "it", "on", /[A-Za-z]+['](d|ve|re|ll|t|s|n)/, /[A-Za-z]+[-][a-z]+/, /[A-Za-z][a-z]{2,}/);
	return f.contains.push({ begin: fe(/[ ]+/, "(", h, /[.]?[:]?([.][ ]|[ ])/, "){3}") }), f;
}, mt = $e("//", "$"), Et = $e("/\\*", "\\*/"), vt = $e("#", "$"), wt = {
	scope: "number",
	begin: Mn,
	relevance: 0
...
function Ln(e, r, { key: l }) {
	let f = 0;
	const h = e[l], n = {}, t = {};
	for (let i = 1; i <= r.length; i++) t[i + f] = h[i], n[i + f] = !0, f += Nn(r[i - 1]);
...
function Kt(e) {
	function r(t, i) {
		return new RegExp(Ae(t), "m" + (e.case_insensitive ? "i" : "") + (e.unicodeRegex ? "u" : "") + (i ? "g" : ""));
	}
	class l {
		constructor() {
	...
	function h(t) {
		const i = new f();
		return t.contains.forEach((s) => i.addRule(s.begin, {
			rule: s,
			type: "begin"
		})), t.terminatorEnd && i.addRule(t.terminatorEnd, { type: "end" }), t.illegal && i.addRule(t.illegal, { type: "illegal" }), i;
	}
	function n(t, i) {
		const s = t;
...
https://github.com/highlightjs/highlight.js/issues/2277`), G = b, U = N), C === void 0 && (C = !0);
	...
	function T(b, N) {
		...
		try {
			...
			if (ue("Language definition for '{}' could not be registered.".replace("{}", b)), h) ue(U);
			else throw U;
			C = t;
		}
...
function Be(e, r, l = "word") {
	return typeof e == "undefined" ? r : typeof r == "undefined" ? e : (l === "char" ? Vn : qn)(e, r).filter((h) => be(h) !== A.DELETE).map((h) => be(h) === A.ADD ? `${ie}${h.value}${se}` : h.value).join("");
}
function $n(e, r) {
...
`);
		if (w === void 0) {
			for (const L of M) {
				let O = i(), P = i();
				const $ = j(l, L);
				D === A.EQUAL && (c++, u++, O = s(A.EQUAL, c, $), P = s(A.EQUAL, u, $)), D === A.DELETE && (c++, O = s(A.DELETE, c, $), P = i()), D === A.ADD && (u++, O = i(), P = s(A.ADD, u, $)), g.push({
					left: O,
					right: P
				});
			}
...
`);
		if (v === A.DELETE) if (w === A.ADD && (D.length === T.length || h)) {
...
var hr = { key: 0 }, dr = /* @__PURE__ */ createBaseVNode("td", {
	...
	setup(e, { emit: r }) {
		...
		return (h, n) => h.line.hideIndex !== void 0 && h.line.hide ? (openBlock(), createElementBlock("tr", hr, [createBaseVNode("td", {
			class: "blob-num blob-num-hunk text-center",
			colspan: "2",
			onClick: n[0] || (n[0] = (t) => l("expand", h.line))
		}, " > "), dr])) : h.line.hide ? createCommentVNode("", !0) : (openBlock(), createElementBlock("tr", pr, [
			createBaseVNode("td", { class: normalizeClass(["blob-num", {
				"blob-num-deletion": h.line.type === unref(A).DELETE,
				"blob-num-addition": h.line.type === unref(A).ADD,
				"blob-num-context": h.line.type === unref(A).EQUAL,
				"blob-num-hunk": h.line.hide !== void 0
			}]) }, toDisplayString(h.line.delNum), 3),
			createBaseVNode("td", { class: normalizeClass(["blob-num", {
				"blob-num-deletion": h.line.type === unref(A).DELETE,
				"blob-num-addition": h.line.type === unref(A).ADD,
				"blob-num-context": h.line.type === unref(A).EQUAL,
				"blob-num-hunk": h.line.hide !== void 0
			}]) }, toDisplayString(h.line.addNum), 3),
			createBaseVNode("td", { class: normalizeClass(["blob-code", {
				"blob-code-deletion": h.line.type === unref(A).DELETE,
				"blob-code-addition": h.line.type === unref(A).ADD,
				"blob-code-context": h.line.type === unref(A).EQUAL,
	...
	setup(e) {
		...
		return (f, h) => {
			var n;
			return openBlock(), createElementBlock("table", mr, [createBaseVNode("tbody", null, [(openBlock(!0), createElementBlock(Fragment, null, renderList((n = f.diffChange) == null ? void 0 : n.changes, (t, i) => (openBlock(), createBlock(br, {
				key: i,
				line: t,
				onExpand: l
			}, null, 8, ["line"]))), 128))])]);
	...
	setup(e, { emit: r }) {
		...
		function h(n) {
			...
			for (const s of i) s.classList.toggle("no-select", n === "left");
		...
		}, " > "), wr])) : n.splitLine.hide ? createCommentVNode("", !0) : (openBlock(), createElementBlock("tr", yr, [(openBlock(!0), createElementBlock(Fragment, null, renderList([n.splitLine.left, n.splitLine.right], (i, s) => (openBlock(), createElementBlock(Fragment, null, [i.type === unref(A).EMPTY ? (openBlock(), createElementBlock(Fragment, { key: 0 }, [Nr, Ar], 64)) : (openBlock(), createElementBlock(Fragment, { key: 1 }, [createBaseVNode("td", { class: normalizeClass(["blob-num", {
			"blob-num-deletion": i.type === unref(A).DELETE,
			"blob-num-addition": i.type === unref(A).ADD,
			"blob-num-context": i.type === unref(A).EQUAL,
			"blob-num-hunk": n.splitLine.hide !== void 0
		}]) }, toDisplayString(i.num), 3), createBaseVNode("td", {
			class: normalizeClass(["blob-code", {
				"blob-code-deletion": i.type === unref(A).DELETE,
				"blob-code-addition": i.type === unref(A).ADD,
				"blob-code-context": i.type === unref(A).EQUAL,
	...
	setup(e) {
		...
		return (f, h) => {
			var n;
			return openBlock(), createElementBlock("table", Sr, [Lr, createBaseVNode("tbody", null, [(openBlock(!0), createElementBlock(Fragment, null, renderList((n = f.diffChange) == null ? void 0 : n.changes, (t, i) => (openBlock(), createBlock(Dr, {
				key: i,
				"split-line": t,
				onExpand: l
...
var bn = /* @__PURE__ */ Ve($r, [["render", xr]]), Fr = ["theme"], zr = {
	...
	emits: ["diff"],
	setup(e, { emit: r }) {
		const l = e, f = r, h = computed(() => l.outputFormat === "line-by-line"), n = computed(() => {
			let g = l.oldString || "";
...
`) : g, g;
			...
			return g = l.trim ? g.trim() : g, g = l.noDiffLineFeed ? g.replace(/(\r\n)/g, `
`) : g, g;
		}), i = computed(() => h.value ? gr(n.value, t.value, l.language, l.diffStyle, l.forceInlineComparison, l.context, l.ignoreMatchingLines) : fr(n.value, t.value, l.language, l.diffStyle, l.forceInlineComparison, l.context, l.ignoreMatchingLines)), s = ref(i.value), o = computed(() => s.value.stat.additionsNum === 0 && s.value.stat.deletionsNum === 0), a = ref(-1);
		function c() {
		...
		}, [g.hideHeader ? createCommentVNode("", !0) : (openBlock(), createElementBlock("div", zr, [unref(h) ? (openBlock(), createElementBlock("div", Gr, [
		...
		])]))])), unref(h) ? (openBlock(), createBlock(Er, {
			key: 1,
			"diff-change": unref(s)
...
var NodeDiff_vue_vue_type_style_index_0_lang_module_default = { codeDiff: "_codeDiff_6ow2z_126" };
...
var SyncedWorkflowCanvas_default = /* @__PURE__ */ defineComponent({
	...
	props: {
		...
		nodes: {},
		connections: {},
		applyLayout: { type: Boolean }
	},
	setup(__props) {
		const props = __props;
		const eventBus = createEventBus();
		const { setViewport, onViewportChange: onLocalViewportChange, onNodeClick, fitView, findNode, addSelectedNodes, onPaneClick, onNodesInitialized, updateNode } = useVueFlow({ id: props.id });
		onNodesInitialized(() => {
			if (props.applyLayout) eventBus.emit("tidyUp", { source: "import-workflow-data" });
		});
		function calculateOriginOffset(nodes) {
			...
			let minX = nodes[0].x;
			let minY = nodes[0].y;
			for (const node of nodes) if (node.y < minY || node.y === minY && node.x < minX) {
				minX = node.x;
				minY = node.y;
			}
			return {
		...
		async function onTidyUp({ result }) {
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("div", _hoisted_1$1, [createVNode(Canvas_default, {
				id: __props.id,
				nodes: __props.nodes,
				connections: __props.connections,
				"read-only": true,
...
var WorkflowDiffAside_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { emit: __emit }) {
		useCssVars((_ctx) => ({ "v3eb5ea38": panelWidth.value }));
		const props = __props;
...
var WorkflowDiffAside_vue_vue_type_style_index_0_lang_module_default = {
	workflowDiffAside: "_workflowDiffAside_1nee2_125",
	nodeTitle: "_nodeTitle_1nee2_134"
};
...
var WorkflowDiffContent_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	__name: "WorkflowDiffContent",
	props: {
		sourceNodes: {},
		sourceConnections: {},
		targetNodes: {},
		...
		selectedNode: {},
		nodeDiffs: {},
		isSourceWorkflowNew: { type: Boolean },
		applyLayout: { type: Boolean },
		nodesDiff: {},
		connectionsDiff: {}
	},
	...
	setup(__props, { emit: __emit }) {
		...
		function getNodeStatusClass(id) {
			return $style[props.nodesDiff?.get(id)?.status ?? "equal"];
		}
		function getEdgeStatusClass(id) {
			return $style[`edge-${props.connectionsDiff?.get(id)?.status ?? NodeDiffStatus.Eq}`];
		}
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("div", { class: normalizeClass(unref($style).workflowDiffContent) }, [createBaseVNode("div", { class: normalizeClass(unref($style).workflowDiff) }, [createBaseVNode("div", { class: normalizeClass(unref($style).workflowDiffPanel) }, [renderSlot(_ctx.$slots, "sourceLabel", {}, () => [createVNode(unref(N8nText_default), {
				color: "text-dark",
				size: "small",
				class: normalizeClass(unref($style).sourceBadge)
			}, {
				default: withCtx(() => [createTextVNode(toDisplayString(__props.sourceLabel), 1)]),
				_: 1
			}, 8, ["class"])]), __props.sourceExists ? (openBlock(), createBlock(SyncedWorkflowCanvas_default, {
				key: 0,
				id: "top",
				nodes: __props.sourceNodes,
				connections: __props.sourceConnections,
				"apply-layout": __props.applyLayout
			}, {
				node: withCtx(({ nodeProps }) => [createVNode(CanvasNode_default, mergeProps(nodeProps, { class: { [getNodeStatusClass(nodeProps.id)]: true } }), {
					toolbar: withCtx(() => [..._cache[1] || (_cache[1] = [])]),
					_: 1
				}, 16, ["class"])]),
				...
				default: withCtx(() => [createTextVNode(toDisplayString(__props.isSourceWorkflowNew ? unref(i18n).baseText("workflowDiff.newWorkflow") : unref(i18n).baseText("workflowDiff.deletedWorkflow")), 1)]),
				_: 1
			}), renderSlot(_ctx.$slots, "sourceEmptyText")], 2))], 2), createBaseVNode("div", { class: normalizeClass(unref($style).workflowDiffPanel) }, [renderSlot(_ctx.$slots, "targetLabel", {}, () => [createVNode(unref(N8nText_default), {
				color: "text-dark",
				size: "small",
				class: normalizeClass(unref($style).sourceBadge)
			}, {
				default: withCtx(() => [createTextVNode(toDisplayString(__props.targetLabel), 1)]),
				_: 1
			}, 8, ["class"])]), __props.targetExists ? (openBlock(), createBlock(SyncedWorkflowCanvas_default, {
				key: 0,
				id: "bottom",
				nodes: __props.targetNodes,
				connections: __props.targetConnections,
				"apply-layout": __props.applyLayout
			}, {
				node: withCtx(({ nodeProps }) => [createVNode(CanvasNode_default, mergeProps(nodeProps, { class: { [getNodeStatusClass(nodeProps.id)]: true } }), {
					toolbar: withCtx(() => [..._cache[2] || (_cache[2] = [])]),
					_: 1
				}, 16, ["class"])]),
				edge: withCtx(({ edgeProps, arrowHeadMarkerId }) => [createVNode(HighlightedEdge_default, mergeProps(edgeProps, {
					"marker-end": `url(#${arrowHeadMarkerId})`,
				...
				default: withCtx(() => [createTextVNode(toDisplayString(unref(i18n).baseText("workflowDiff.deletedWorkflow")), 1)]),
				_: 1
			}), renderSlot(_ctx.$slots, "targetEmptyText")], 2))], 2)], 2), __props.selectedNode ? (openBlock(), createBlock(WorkflowDiffAside_default, {
				key: 0,
				node: __props.selectedNode,
				onClose: _cache[0] || (_cache[0] = ($event) => emit("closeAside"))
			}, {
				default: withCtx(({ outputFormat }) => [createVNode(NodeDiff_default, mergeProps(__props.nodeDiffs, { "output-format": outputFormat }), null, 16, ["output-format"])]),
				_: 1
			}, 8, ["node"])) : createCommentVNode("", true)], 2);
		};
	}
});
var WorkflowDiffContent_vue_vue_type_style_index_0_lang_module_default = {
	workflowDiffContent: "_workflowDiffContent_p1w5q_125",
	workflowDiff: "_workflowDiff_p1w5q_125",
	workflowDiffPanel: "_workflowDiffPanel_p1w5q_139",
	sourceBadge: "_sourceBadge_p1w5q_146 _sourceBadge_5j5ev_3",
	emptyWorkflow: "_emptyWorkflow_p1w5q_150",
	deleted: "_deleted_p1w5q_159 _deleted_5j5ev_20",
	added: "_added_p1w5q_163 _added_5j5ev_21",
	equal: "_equal_p1w5q_167 _equal_5j5ev_95",
	modified: "_modified_p1w5q_171 _modified_5j5ev_22",
	"edge-deleted": "_edge-deleted_p1w5q_176 _edge-deleted_5j5ev_136",
	"edge-added": "_edge-added_p1w5q_180 _edge-added_5j5ev_141",
	"edge-equal": "_edge-equal_p1w5q_184 _edge-equal_5j5ev_146"
};
var WorkflowDiffContent_default = /* @__PURE__ */ __plugin_vue_export_helper_default(WorkflowDiffContent_vue_vue_type_script_setup_true_lang_default, [["__cssModules", { "$style": WorkflowDiffContent_vue_vue_type_style_index_0_lang_module_default }]]);
...
const useWorkflowDiff = (sourceWorkflow, targetWorkflow) => {
	...
	const sourceDiff = createWorkflowDiff(sourceRefs.workflowRef, sourceRefs.workflowNodes, sourceRefs.workflowConnections, sourceRefs.workflowObjectRef);
	const targetDiff = createWorkflowDiff(targetRefs.workflowRef, targetRefs.workflowNodes, targetRefs.workflowConnections, targetRefs.workflowObjectRef);
...
function useWorkflowDiffUI(options) {
	const { sourceWorkflow, targetWorkflow, nodesDiff, connectionsDiff, selectedDetailId, onNavigate, onTabChange } = options;
	const nodeTypesStore = useNodeTypesStore();
	const i18n = useI18n();
	const settingsDiff = computed(() => {
		const sourceSettings = sourceWorkflow.value?.settings ?? {};
		const targetSettings = targetWorkflow.value?.settings ?? {};
		...
		const sourceTags = (sourceWorkflow.value?.tags ?? []).map((tag) => typeof tag === "string" ? tag : tag.name);
		const targetTags = (targetWorkflow.value?.tags ?? []).map((tag) => typeof tag === "string" ? tag : tag.name);
		if (JSON.stringify(sourceTags) !== JSON.stringify(targetTags)) settings.push({
			name: "tags",
	...
	const nodeDiffs = computed(() => {
		...
		const sourceNode = sourceWorkflow.value?.nodes.find((node) => node.id === selectedDetailId.value);
		const targetNode = targetWorkflow.value?.nodes.find((node) => node.id === selectedDetailId.value);
	...
	function setSelectedDetailId(nodeId) {
...
export { NodeDiff_default as a, useProvideViewportSync as i, useWorkflowDiff as n, DiffBadge_default as o, WorkflowDiffContent_default as r, useWorkflowDiffUI as t };
