...
import { C as computed, Cn as toDisplayString, D as createElementBlock, E as createCommentVNode, G as nextTick, Gt as unref, It as ref, M as createVNode, P as defineComponent, T as createBlock, W as mergeProps, _ as Fragment, at as renderSlot, bt as withCtx, ct as resolveDynamicComponent, et as openBlock, it as renderList, j as createTextVNode, vn as normalizeClass, w as createBaseVNode, zt as shallowRef } from "./vue.runtime.esm-bundler-XtMkEjzB.js";
import { M as useLocalStorage, S as useClipboard, yt as useI18n } from "./_MapCache-CrDDrJ83.js";
import { Ei as require_markdown_it_link_attributes, bi as useRoute, mt as createEventBus, xi as useRouter } from "./src-BsVsetYf.js";
import { t as __plugin_vue_export_helper_default } from "./_plugin-vue_export-helper-DltO58Gh.js";
import { An as doRectsOverlap, Ar as usePostHog, Bt as useNDVStore, C as useNodeTypesStore, Dn as GRID_SIZE, En as DEFAULT_VIEWPORT_BOUNDARIES, Gt as useProjectsStore, Ht as useWorkflowState, In as getNewNodePosition, It as useNodeHelpers, Lr as v4_default, Lt as useTelemetry, On as PUSH_NODES_OFFSET, Ot as useTagsStore, R as removePreviewToken, Rn as getNodesGroupSize, Sn as CONFIGURABLE_NODE_SIZE, Sr as useCanvasStore, Tn as DEFAULT_NODE_SIZE, Tt as useWorkflowHelpers, Vt as injectWorkflowState, _i as RemoveConnectionCommand, aa as isPresent, bi as ReplaceNodeParametersCommand, ct as useDataSchema, d as isChatNode, di as AddConnectionCommand, fi as AddNodeCommand, gi as MoveNodeCommand, gn as useFocusPanelStore, jn as generateOffsets, jt as useWorkflowDocumentStore, kt as createWorkflowDocumentId, li as CanvasNodeRenderType, r as useUIStore, ri as isValidNodeConnectionType, rt as useCredentialsStore, s as useWorkflowsStore, si as CanvasConnectionMode, ua as tryToParseNumber, ui as useHistoryStore, vi as RemoveNodeCommand, wn as CONFIGURATION_NODE_SIZE, xn as useTemplatesStore, xr as useExternalHooks, xs as useSettingsStore, y as getExecutionErrorToastConfiguration, yi as RenameNodeCommand, yr as useToast } from "./users.store-yKax6vGx.js";
import { $s as LOCAL_STORAGE_LOGS_PANEL_DETAILS_PANEL, Ai as require_cloneDeep, Ba as deepCopy, Gc as EnterpriseEditionFeature, Mn as MCP_TRIGGER_NODE_TYPE, Oc as CANVAS_ZOOMED_VIEW_EXPERIMENT, Pc as NDV_IN_FOCUS_PANEL_EXPERIMENT, Sr as WEBHOOK_NODE_TYPE, Va as isCommunityPackageName, ao as NodeConnectionTypes, br as UPDATE_WEBHOOK_ID_NODE_TYPES, ca as getNodeInputs, ec as LOCAL_STORAGE_LOGS_PANEL_DETAILS_PANEL_SUB_NODE, ha as getSubworkflowId, hn as FORM_TRIGGER_NODE_TYPE, ia as generateNodesGraph, la as getNodeOutputs, mr as STICKY_NODE_TYPE, nc as LOCAL_STORAGE_LOGS_SYNC_SELECTION, ns as VIEWS, sa as getConnectionTypes, tc as LOCAL_STORAGE_LOGS_PANEL_OPEN, ua as getNodeParameters, ya as isHitlToolType, zi as require__baseUniq } from "./constants-CtA9jyV-.js";
import { j as defineStore, r as useRootStore } from "./_baseOrderBy-B5lXBmMF.js";
import { t as useExecutionsStore } from "./executions.store-R46IFWXG.js";
import { t as usePinnedData } from "./usePinnedData-6cs4oFEc.js";
import { a as mapLegacyConnectionToCanvasConnection, c as parseCanvasConnectionHandleString, i as mapCanvasConnectionToLegacyConnection, n as createCanvasConnectionHandleString, o as mapLegacyConnectionsToCanvasConnections } from "./canvas.utils-CaIgedg0.js";
import { t as useNodeCreatorStore } from "./nodeCreator.store-DRjAjMk5.js";
import { t as core_default } from "./core-VADtcMBO.js";
import { a as bash, i as javascript, n as typescript, r as python, t as xml } from "./xml-BMqNMrYo.js";
import { t as VueMarkdown_default } from "./VueMarkdown-CI4PiEb6.js";
import { t as useFoldersStore } from "./folders.store-Dx3ZI_kr.js";
...
var MarkdownRenderer_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props) {
		...
		return (_ctx, _cache) => {
			return openBlock(), createBlock(unref(VueMarkdown_default), {
	...
	setup(__props) {
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("div", null, [createVNode(MarkdownRenderer_default, { text: __props.text }, null, 8, ["text"]), createBaseVNode("div", { class: normalizeClass(_ctx.$style.buttons) }, [(openBlock(true), createElementBlock(Fragment, null, renderList(__props.buttons, (button, index) => {
				return openBlock(), createElementBlock(Fragment, { key: button.text }, [clickedButtonIndex.value === null || index === clickedButtonIndex.value ? (openBlock(), createBlock(Button_default, {
					key: 0,
					element: "button",
					type: button.type,
...
var MessageWithButtons_default = /* @__PURE__ */ __plugin_vue_export_helper_default(MessageWithButtons_vue_vue_type_script_setup_true_lang_default, [["__cssModules", { "$style": MessageWithButtons_vue_vue_type_style_index_0_lang_module_default }]]);
const MessageComponentKey = { WITH_BUTTONS: "with-buttons" };
...
function constructChatWebsocketUrl(url, executionId, sessionId, isPublic) {
	const baseUrl = new URL(url).origin;
	const wsProtocol = baseUrl.startsWith("https") ? "wss" : "ws";
	return `${baseUrl.replace(/^https?/, wsProtocol)}/chat?sessionId=${sessionId}&executionId=${executionId}${isPublic ? "&isPublic=true" : ""}`;
}
...
function useCanvasOperations() {
			...
			if (newNodeInsertPosition) {
				...
				if (isHitlToolType(node.type) && lastInteractedWithNodeConnection && connectionType === NodeConnectionTypes.AiTool) {
					...
					if (toolUserNode) {
						...
						position = [lastInteractedWithNode.position[0] - CONFIGURABLE_NODE_SIZE[0] / 2, middleY];
						if (Math.abs(middleY - targetY) < PUSH_NODES_OFFSET) {
							const verticalOffset = PUSH_NODES_OFFSET / 2;
							updateNodePosition(lastInteractedWithNode.id, {
								x: lastInteractedWithNode.position[0],
								y: lastInteractedWithNode.position[1] + (yDiff > 0 ? verticalOffset : -verticalOffset)
							}, { trackHistory: true });
						}
				...
				if (outputTypes.length > 0 && outputTypes.every((outputName) => outputName !== NodeConnectionTypes.Main)) {
					...
					if (lastInteractedWithNodeConnection) position = [position[0], lastInteractedWithNode.position[1]];
				}
			}
		}
	...
	function getNodesToShift(insertPosition, sourceNodeName, nodeSize = DEFAULT_NODE_SIZE) {
		...
		for (const candidate of initialCandidates) workflowHelpers.getConnectedNodes("downstream", editableWorkflowObject.value, candidate.name).filter((name) => {
		...
		for (const sticky of stickyNodes) {
			...
			if (sourceNode && isNodeAssociatedWithSticky(sourceNode, sticky, stickyRect, stickyCenter, nodeCenterThreshold) && isInsertionInsideSticky) {
				const associatedNodes$1 = getAssociatedNodes(sticky, stickyRect, stickyCenter, nodeCenterThreshold, regularNodesToMove);
				stickyAssociatedNodes.set(sticky.id, associatedNodes$1);
	...
	async function addImportedNodesToWorkflow(data, { trackBulk = true, trackHistory = false, viewport = DEFAULT_VIEWPORT_BOUNDARIES, setStateDirty = true } = {}) {
		...
		const tempWorkflow = workflowsStore.createWorkflowObject(createNodes, newConnections, true);
	...
	async function importWorkflowTags(workflowData) {
		...
		useWorkflowDocumentStore(createWorkflowDocumentId(workflowsStore.workflowId)).addTags(tagIds);
	}
	...
	async function openExecution(executionId, nodeId) {
		...
		if (!["manual", "evaluation"].includes(data.mode)) workflowState.setWorkflowPinData({});
		if (nodeId) {
			const node = workflowsStore.getNodeById(nodeId);
...
export { MessageComponentKey as _, useLogsStore as a, LOCAL_STORAGE_PANEL_WIDTH as c, chatEventBus as d, constructChatWebsocketUrl as f, ChatSymbol as g, ChatOptionsSymbol as h, useExperimentalNdvStore as i, LOGS_PANEL_STATE as l, shouldBlockUserInput as m, useParentFolder as n, LOCAL_STORAGE_OVERVIEW_PANEL_WIDTH as o, parseBotChatMessageContent as p, canvasEventBus as r, LOCAL_STORAGE_PANEL_HEIGHT as s, useCanvasOperations as t, LOG_DETAILS_PANEL_STATE as u, MessageWithButtons_default as v, MarkdownRenderer_default as y };
