...
let ChatHubWorkflowService = class ChatHubWorkflowService {
    ...
    async createChatWorkflow(userId, sessionId, projectId, history, humanMessage, attachments, credentials, model, systemMessage, tools, timeZone, executionMetadata, trx) {
    ...
    async buildChatWorkflow({ userId, sessionId, history, humanMessage, attachments, credentials, model, systemMessage, tools, executionMetadata, }) {
        ...
        const nodeExecutionStack = this.prepareExecutionData(chatTriggerNode, sessionId, humanMessage, attachments, executionMetadata);
...
`;
    ...
    getBaseSystemMessage(history, timeZone) {
...
Respond the title only:`,
    ...
    prepareExecutionData(triggerNode, sessionId, message, attachments, executionMetadata) {
                ...
                node: {
                    ...
                    parameters: {
                        ...
                        contextEstablishmentHooks: {
                            hooks: [
                                {
                                    hookName: chat_hub_extractor_1.CHATHUB_EXTRACTOR_NAME,
    ...
    async prepareReplyWorkflow(user, sessionId, credentials, model, history, message, tools, attachments, timeZone, trx, executionMetadata) {
        if (model.provider === 'n8n') {
            return await this.prepareWorkflowAgentWorkflow(user, sessionId, model.workflowId, message, attachments, trx, executionMetadata);
        }
        if (model.provider === 'custom-agent') {
            return await this.prepareChatAgentWorkflow(model.agentId, user, sessionId, history, message, attachments, timeZone, trx, executionMetadata);
        }
    ...
    async prepareChatAgentWorkflow(agentId, user, sessionId, history, message, attachments, timeZone, trx, executionMetadata) {
    ...
    async prepareWorkflowAgentWorkflow(user, sessionId, workflowId, message, attachments, trx, executionMetadata) {
        ...
        const nodeExecutionStack = this.prepareExecutionData(chatTrigger, sessionId, message, attachments, executionMetadata);
        const executionData = (0, n8n_workflow_1.createRunExecutionData)({
            executionData: {
                nodeExecutionStack,
            },
    ...
    buildArtifactContext(history) {
        const artifacts = (0, chat_hub_1.collectChatArtifacts)(history.flatMap(chat_hub_1.parseMessage));
