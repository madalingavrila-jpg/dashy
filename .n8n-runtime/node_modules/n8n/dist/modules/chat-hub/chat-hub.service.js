...
let ChatHubService = class ChatHubService {
    constructor(logger, errorReporter, executionRepository, workflowFinderService, sessionRepository, messageRepository, chatHubAgentService, chatHubModelsService, chatHubAttachmentService, chatStreamService, chatHubExecutionService, chatHubTitleService, chatHubWorkflowService, globalConfig) {
        this.logger = logger;
        ...
        try {
            result = await this.messageRepository.manager.transaction(async (trx) => {
                ...
                if (messageToEdit.type === 'human') {
                    ...
                    const workflow = await this.chatHubWorkflowService.prepareReplyWorkflow(user, sessionId, credentials, model, history, message, session.tools, attachments, tz, trx, executionMetadata);
    ...
    async executeChatWorkflowWithCleanup(user, model, workflow, sessionId, previousMessageId, retryOfMessageId, originalPreviousMessageId, credentials, humanMessage, processedAttachments) {
        await this.chatHubExecutionService.executeChatWorkflowWithCleanup(user, model, workflow.workflowData, workflow.executionData, sessionId, previousMessageId, retryOfMessageId, workflow.responseMode);
