...
let CredentialsHelper = class CredentialsHelper extends n8n_workflow_1.ICredentialsHelper {
    constructor(credentialTypes, credentialsOverwrites, credentialsRepository, sharedCredentialsRepository, cacheService, dynamicCredentialsProxy) {
        super();
    ...
    async getDecrypted(additionalData, nodeCredentials, type, mode, executeData, raw, expressionResolveValues) {
        ...
        if (additionalData.executionContext?.credentials !== undefined) {
            ...
            }, decryptedDataOriginal, additionalData.executionContext, additionalData.workflowSettings, canUseExternalSecrets);
        }
        if (raw === true) {
            return decryptedDataOriginal;
        }
        return await this.applyDefaultsAndOverwrites(additionalData, decryptedDataOriginal, type, mode, canUseExternalSecrets, executeData, expressionResolveValues);
    }
    async applyDefaultsAndOverwrites(additionalData, decryptedDataOriginal, type, mode, canUseExternalSecrets, executeData, expressionResolveValues) {
