...
let OauthService = class OauthService {
    ...
    async applyDefaultsAndOverwrites(credential, decryptedData, additionalData) {
        const canUseExternalSecrets = await this.credentialsHelper.credentialCanUseExternalSecrets(credential);
        return (await this.credentialsHelper.applyDefaultsAndOverwrites(additionalData, decryptedData, credential.type, 'internal', canUseExternalSecrets, undefined, undefined));
    }
    ...
    async generateAOauth2AuthUri(credential, csrfData) {
        ...
        if (oauthCredentials.useDynamicClientRegistration && oauthCredentials.serverUrl) {
            const serverUrl = oauthCredentials.serverUrl.replace(/\/$/, '');
            const { data } = await axios_1.default.get(`${serverUrl}/.well-known/oauth-authorization-server`);
    ...
    async saveDynamicCredential(credential, oauthTokenData, authHeader, credentialResolverId) {
        const credentials = new n8n_core_1.Credentials(credential, credential.type, credential.data);
        ...
        await this.dynamicCredentialsProxy.storeIfNeeded(credentialStoreMetadata, oauthTokenData, { version: 1, identity: authHeader }, credentials.getData(), { credentialResolverId });
    }
};
