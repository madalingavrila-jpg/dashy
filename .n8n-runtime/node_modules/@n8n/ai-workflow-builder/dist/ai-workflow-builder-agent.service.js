...
let AiWorkflowBuilderService = class AiWorkflowBuilderService {
    ...
    nodeTypes;
    sessionManager;
    constructor(parsedNodeTypes, client, logger, instanceId, instanceUrl, n8nVersion, onCreditsUpdated, onTelemetryEvent, resourceLocatorCallbackFactory) {
        this.client = client;
        this.logger = logger;
        ...
        this.sessionManager = new session_manager_service_1.SessionManagerService(this.nodeTypes, logger);
    }
    ...
    async *chat(payload, user, abortSignal) {
        ...
        if (pendingHitl && payload.resumeData) {
            if (pendingHitl.value.type === 'questions') {
                ...
                const decision = payload.resumeData;
    ...
    async trackBuilderReplyTelemetry(agent, workflowId, userId, userMessageId) {
        if (!this.onTelemetryEvent)
            return;
        const state = await agent.getState(workflowId, userId);
        const threadId = session_manager_service_1.SessionManagerService.generateThreadId(workflowId, userId);
        const lastAiMessage = state.values.messages.findLast((m) => m instanceof messages_1.AIMessage);
        const messageAi = typeof lastAiMessage?.content === 'string'
            ? lastAiMessage.content
            : JSON.stringify(lastAiMessage?.content ?? '');
        const toolMessages = state.values.messages.filter((m) => m instanceof messages_1.ToolMessage);
        const toolsCalled = [
...
exports.AiWorkflowBuilderService = AiWorkflowBuilderService = AiWorkflowBuilderService_1 = __decorate([
    ...
    __metadata("design:paramtypes", [Array, ai_assistant_sdk_1.AiAssistantClient,
        backend_common_1.Logger, String, String, String, Function, Function, Function])
