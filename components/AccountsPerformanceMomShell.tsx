...
export function AccountsPerformanceMomShell() {
  ...
  // Per-cohort summary (launch → date totals) for the selected month.
  const summary = useMemo(() => {
    const gmv = cohortAccounts.reduce((s, a) => s + a.totalGmv, 0);
    const orders = cohortAccounts.reduce((s, a) => s + a.totalOrders, 0);
    const commission = cohortAccounts.reduce((s, a) => s + (a.totalCommission ?? 0), 0);
    return {
  ...
  // Comparison across activation-month cohorts (oldest → newest for the chart).
  const cohortBars = useMemo(() => {
    return [...cohortMonths]
      ...
      .map((month) => {
        ...
        const gmv = list.reduce((s, a) => s + a.totalGmv, 0);
        const orders = list.reduce((s, a) => s + a.totalOrders, 0);
  ...
  const cards = [
    {
      ...
      icon: "storefront",
      hint: `Accounts launched in ${selectedMonth ? monthLabel(selectedMonth) : "—"}`,
    },
    {
      label: "GMV generated",
      value: formatEurCompact(summary.gmv),
      icon: "payments",
      hint: "Gross GMV (before discounts) generated by this cohort since launch",
    },
    { label: "Orders", value: formatInteger(summary.orders), icon: "receipt_long", hint: "Delivered orders since launch" },
    {
      label: "Commission",
      value: formatEurCompact(summary.commission),
      icon: "percent",
      hint: "Commission summed across the cohort: Salesforce rate (Opportunity.Commission__c) × gross GMV, or the actual Databricks commission when an account has no SF rate",
    },
    {
      label: "AOV gross",
      value: formatEur(summary.aov),
      icon: "shopping_basket",
      hint: "Gross GMV ÷ delivered orders, cohort-wide",
    },
  ];
  ...
  const subtitle = ap
    ? `Activation cohorts · ${ap.country} · pick an activation month to see the accounts that launched that month and the gross GMV (before discounts), orders, AOV, commission & quality they have generated since launch`
    : "Pick an activation month to see the accounts that launched that month and what they have generated since launch.";
    ...
    <div className="mx-auto max-w-[1500px] space-y-md">
      ...
      <section className="glass-card rounded-xl p-lg">
        <h3 className="mb-md text-title-md font-bold text-on-surface">
          ...
          <span className="text-label-md font-normal text-on-surface-variant">
            (launch → date · click a month to drive the table)
          </span>
        </h3>
        {cohortBars.length ? (
      ...
      <section className="space-y-sm">
        <div className="flex items-center justify-between">
          <p className="text-label-md font-semibold uppercase tracking-wide text-primary">
            Accounts activated · {selectedMonth ? monthLabel(selectedMonth) : "—"} cohort · launch → date
          </p>
