...
export const REACTIVATION_EVENT_DATE_OVERRIDES = Object.freeze({
  ...
  // restaurant actually reactivated on 2026-09-03 (provider 78104 first
  // delivered order after the July re-win).
  "006Qs00000krfkAIAQ": "2026-09-03",
  // Taqueria by El Torito — same July re-win batch, same stale
  // Reactivated_Date__c 2026-07-31; provider 78180 went live 2026-09-04.
  "006Qs00000krbN8IAI": "2026-09-04",
  // Banjo Jack — same July re-win batch and stale 2026-07-31 date. Provider
  // 78105 is back online in September but has taken no orders yet (last
  // delivered order 2026-04-02), so it is dated with the group go-live.
  "006Qs00000krz9JIAQ": "2026-09-03",
});

/**
 * Accounts that must never receive Activated credit, even if Salesforce later
 * gains a first-active/reactivation date or an Activated stage transition.
 */
export const EXCLUDED_ACTIVATION_ACCOUNT_IDS = Object.freeze({
  // Comoara Cu Clatite - Mosnita — opportunity 006Qs00000m167mIAA is Closed
  // Lost and the account must not count as an Eusebiu Hanganu activation.
  "001Qs00000u3h1pIAA": "Comoara Cu Clatite - Mosnita — Eusebiu Hanganu",
});

export function isExcludedActivationAccount(recordOrRecords) {
  const records = Array.isArray(recordOrRecords) ? recordOrRecords : [recordOrRecords];
  return records.some((record) => {
    const accountId = record?.AccountId ?? record?.Account?.Id;
    return Boolean(accountId) && accountId in EXCLUDED_ACTIVATION_ACCOUNT_IDS;
  });
}
