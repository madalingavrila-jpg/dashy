...
export function buildMonthlyByProvider(monthlyRows) {
  ...
  for (const [providerIdRaw, month, gmv, orders, commission, gmvNet, discount] of monthlyRows) {
...
export function buildQualityByProvider(qualityRows) {
  ...
  for (const row of qualityRows) {
    const [providerIdRaw, month, orders, av, aw, cv, cw, rjv, rjw, pv, pw, rtv, rtw, ltv, ltw] = row;
    const providerId = String(providerIdRaw);
...
export function qualityForProvider(qualityByProvider, providerId, launchMonth) {
  const rows = (qualityByProvider.get(String(providerId)) ?? []).filter(
...
export function assembleAccount(
  ...
  const launchMonthly = (monthlyByProvider.get(String(providerId)) ?? [])
    .slice()
