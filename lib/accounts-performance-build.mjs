...
export function assembleAccount(
  row,
  { agentId, agentName, segment, monthlyByProvider, qualityByProvider, commissionRateByProvider },
) {
  const [
    providerId,
    ,
  ...
  // Commission is the Salesforce negotiated rate (Opportunity.Commission__c, %);
  // commission € is derived as rate × GROSS GMV. Null when SF has no rate.
  const commissionRatePct = commissionRateByProvider?.get(String(providerId)) ?? null;
  const commissionEur = (gmv) =>
    commissionRatePct == null ? null : Math.round((commissionRatePct / 100) * gmv);
  ...
  const monthly = (monthlyByProvider.get(providerId) ?? [])
    ...
    .map((m) => {
      ...
      return {
        ...
        // commission € = SF rate × gross GMV (null when no SF rate).
        commission: commissionEur(gmv),
  ...
  const totalCommission = commissionEur(totalGmv);
