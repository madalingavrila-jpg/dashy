 ...
 *
 * @typedef {{ ownerId: string, segment: "complex" | "density", alias: string, sfName?: string, hiddenFromMonth?: string, match: (normalizedName: string) => boolean }} RosterEntry
 * @type {RosterEntry[]}
 */
...
const HIDDEN_FROM_MONTH = Object.freeze(
  Object.fromEntries(
    TEAM_ROSTER.filter((r) => r.hiddenFromMonth).map((r) => [r.ownerId, r.hiddenFromMonth]),
  ),
);

/**
 * True when this roster owner should be omitted from UI for `monthKey`
 * (YYYY-MM) and later. Historical months before `hiddenFromMonth` still show.
 */
export function isHiddenFromRoster(ownerId, monthKey) {
  if (!ownerId || !monthKey) return false;
  const from = HIDDEN_FROM_MONTH[ownerId];
  return Boolean(from) && monthKey >= from;
}
