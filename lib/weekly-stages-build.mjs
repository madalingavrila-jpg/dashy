...
import {
  dedupeActivationByAccount,
  pickPrimaryActivationOpp,
  firstActivatedTransitionIndex,
  reactivationEventDate,
} from "./mtd-history.mjs";
import { resolveAccountCity } from "./city-overrides.mjs";
import {
  forcedReactivationOpps,
  isExcludedActivationAccount,
} from "./activation-overrides.mjs";
...
function accountEntry(rec, eventDate) {
  return {
    ...
    city: resolveAccountCity(rec.AccountId, rec.Account?.BillingCity),
    stage: rec.StageName,
    date: formatEventDate(eventDate),
    sfOpportunityId: rec.Id,
    sfAccountId: rec.AccountId ?? undefined,
    ...(rec.reactivated ? { reactivated: true } : {}),
  };
}
...
export function accumulateWeeklyActiveFromActivationDate(
  ...
  for (const opps of byAccount.values()) {
    if (isExcludedActivationAccount(opps)) continue;
...
/**
 * Weekly "Active" — REACTIVATIONS. Accounts whose provider_first_active_date__c
 * is BEFORE the tracking year but which have a tracking-year won Sales
 * Opportunity or won RecordType=Reactivation opp (scripts/.cache/sf-reactivation-*).
 * Same dedup/attribution as the base Active bucket; week = ISO week
 * (Europe/Bucharest) of the reactivation event date — mirrors MTD
 * accumulateMtdReactivated.
 */
export function accumulateWeeklyReactivations(
  reactivationRecords,
  historyRecords,
  weekStore,
  agentSegmentFn,
  ...
  const transitionIndex = firstActivatedTransitionIndex(historyRecords, TRACKING_YEAR);
  ...
  for (const opps of byAccount.values()) {
    ...
    if (!firstActive || Number(firstActive.slice(0, 4)) >= TRACKING_YEAR) continue;
    ...
    applyEvent(weekStore, segment, ownerId, "active", rec, eventDate);
  }
}

/**
 * Weekly "Active" for FORCED reactivations (lib/activation-overrides.mjs) — the
 * weekly twin of accumulateMtdForcedReactivations. Same-tracking-year re-wins are
 * read from the base activation export and bucketed by the ISO week of the forced
 * opportunity's OWN first transition INTO 'Activated'.
 */
export function accumulateWeeklyForcedReactivations(
  activationRecords,
  historyRecords,
  weekStore,
  agentSegmentFn,
  ...
  for (const opp of forcedReactivationOpps(activationRecords)) {
    if (isExcludedActivationAccount(opp)) continue;
