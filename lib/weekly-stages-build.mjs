...
/**
 * Weekly "Closed Won" now mirrors the canonical MTD Won definition
 * (Won_Date__c set, RecordType = 'Sales Opportunity', team reps) bucketed by ISO
 * week — see accumulateWeeklyClosedWonFromWonDate. It is NO LONGER derived from
 * field-history "Closed Won" stage transitions, so "Closed Won" is intentionally
 * absent from STAGE_TO_WEEKLY_STATUS below.
 */
export const WEEKLY_ACTIVE_STAGES = ["Activated"];

/**
 * SF stage → weekly status bucket (first transition INTO stage counts).
 * NOTE: "Closed Won" is deliberately excluded here — weekly Won is computed from
 * Won_Date__c (accumulateWeeklyClosedWonFromWonDate) to match the rest of the app.
 */
