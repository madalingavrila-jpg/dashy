...
import { isWonMtdOpportunity } from "./mtd-history.mjs";
...
/** Aligns with MTD Won — current stage Contract sent / Ready to Activate, week from CloseDate. */
export const WEEKLY_CLOSED_WON_STAGES = ["Contract sent", "Ready to Activate"];
...
const RECORD_TYPES_BY_STATUS = {
  ...
  closedWon: new Set(["Sales Opportunity"]),
...
/**
 * Weekly Closed Won from CloseDate (same rules as MTD Won).
 * ISO week = CloseDate in Europe/Bucharest; one row per opportunity.
 */
export function accumulateWeeklyClosedWonFromCloseDate(records, weekStore, agentSegmentFn, isExcludedFn) {
  for (const rec of records ?? []) {
    if (!isWonMtdOpportunity(rec)) continue;
    ...
    const eventDate = parseSfDate(rec.CloseDate);
    applyEvent(weekStore, segment, ownerId, "closedWon", rec, eventDate);
  }
}
 ...
 * Closed Won is excluded — use accumulateWeeklyClosedWonFromCloseDate instead.
 */
