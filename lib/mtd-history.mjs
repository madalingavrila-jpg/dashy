 ...
 *   3. null — the account has not gone live yet (won but still onboarding);
 ...
 * year won Sales Opportunity whose `provider_first_active_date__c` is BEFORE the
 * tracking year (so the base activation accumulator skips it). Same dedup (one
 * per account) and owner attribution as base Activated; dated by
 * reactivationEventDate (field-history first INTO 'Activated', Won_Date fallback).
 * Items are flagged `reactivated: true` for the drill-down lists.
 */
