...
/** SF dashboard 01ZTs000000L8AfMAK — Won MTD stages (Sales Opportunity). */
export const WON_STAGES_MTD = ["Contract sent", "Ready to Activate"];
...
/**
 * SF dashboard 01ZTs000000L8AfMAK alignment:
 * CloseDate THIS_MONTH + Sales Opportunity + current stage Contract sent / RTA only.
 * Opps that advanced to Activated (or Onboarding) drop out even if Won_Date__c is this month.
 * Do not filter on Won_Date__c — Contract sent rows often have null Won_Date__c.
 */
...
export function buildHybridMtdStore(wonRecords, historyRecords) {
  ...
  for (const [monthKey, closeAgents] of closeWonStore) {
    ...
    // CloseDate export is authoritative — drop field-history won for this month.
