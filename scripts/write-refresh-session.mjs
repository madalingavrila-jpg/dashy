#!/usr/bin/env node
/** Session one-shot: write MCP pull results into scripts/.cache/ */
...
// Inline/small pulls saved to agent-tools
writeSf("sf-weekly-2026-07.json", readJson(join(tools, "weekly-july.json")));
writeSf("sf-reactivation-2026.json", readJson(join(tools, "reactivation-team.json")));
writeSf("sf-inbound-reactivation-2026.json", readJson(join(tools, "reactivation-inbound.json")));
writeSf("sf-inbound-won-mtd.json", readJson(join(tools, "inbound-won-mtd.json")));

const sc = readJson(join(tools, "stage-counts.json"));
...
const mp = assembleMpTotals(readJson(join(tools, "mp-totals-opps.json")), readJson(join(tools, "mp-totals-leads.json")));
writeFileSync(join(cacheDir, "mp-totals.json"), `${JSON.stringify(mp, null, 2)}\n`);
console.log("wrote mp-totals.json");

const mops = assembleMops(readJson(join(tools, "960d25bc-3eb0-4036-b4c9-da716f497394.txt")));
...
writeMcpTable("accounts-perf-accounts.json", "RO providers activated YTD (won Sales Opportunity), with activation opp owner. Universe: dim_provider_opportunity ⨝ dim_provider_v2, provider_activated_ts >= 2026-01-01, country Romania, ROW_NUMBER dedup.", parseDb(join(tools, "abbcd3e2-80af-4822-8b20-876c7c27dc16.txt")));
writeMcpTable("accounts-perf-prov-opp.json", "Provider → won opportunity map (same window/dedup as accounts-perf-accounts).", parseDb(join(tools, "cec6cff5-5850-46a4-9b09-b50b773e9f9e.txt")));
...
const commRecords = [];
for (let i = 1; i <= 6; i++) {
  try { commRecords.push(...(readJson(join(tools, `sf-comm-${i}-result.json`)).records ?? [])); } catch { /* skip */ }
}
...
console.log("write-refresh-session done");
