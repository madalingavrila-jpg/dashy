  ...
  entries.push(
    {
      ...
      note: "Monthly GROSS GMV / orders / commission per provider. IN-list comes from accounts-perf-accounts.json — run AFTER batch C1 lands. Watch the 10,000-row Databricks MCP cap (use --chunk=<n> and merge; merged file may exceed 10k).",
      format: "mcp-table",
      bounds: [50, 25000],
      cap: null,
      refreshedEachRun: true,
    ...
    },
    {
      ...
      note: "Monthly availability & performance value/weight pairs per provider. Same IN-list dependency and chunk+merge as monthly (merged file may exceed 10k).",
      format: "mcp-table",
      bounds: [50, 25000],
      cap: null,
      refreshedEachRun: true,
    ...
    {
      file: "scripts/.cache/accounts-perf-daily.json",
      batch: "C2",
      source: "databricks-mcp",
      query: null,
      gen: "node scripts/gen-accounts-perf-queries.mjs --kind=daily",
      note: "OPTIONAL — current-month DAILY GMV / orders / commission per provider. Only needed while the build flags a Databricks month as corrupt (see lib/accounts-performance-anomaly.mjs): with it the month is rebuilt from its clean days instead of suppressed. Same IN-list dependency as monthly.",
      format: "mcp-table",
      bounds: [50, 60000],
      cap: null,
      refreshedEachRun: false,
      closedMonthChunk: false,
      optional: true,
    },
    {
